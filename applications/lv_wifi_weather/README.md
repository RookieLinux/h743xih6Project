...
- 首页系统更新卡片、可编辑 OTA 服务器 IPv4、固件版本/更新说明显示及非阻塞确认回调；
...
右侧系统更新卡片可通过数字键盘输入 MQTT 服务器 IPv4。应用层注册
`lvww_set_server_address_callback()` 后，用户点击“应用”即可校验地址并触发
OTA 控制器使用 `tcp://<IPv4>:1883` 重新连接。
