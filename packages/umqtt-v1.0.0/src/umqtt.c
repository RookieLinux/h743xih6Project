...
_reconnect:
    ...
    if ((client->mqtt_info.lwt_topic != RT_NULL) &&
        (client->mqtt_info.lwt_message != RT_NULL))
    {
        encode_msg.msg.connect.connect_flags.bits.will_flag = 1;
        encode_msg.msg.connect.connect_flags.bits.will_Qos =
            client->mqtt_info.lwt_qos;
        encode_msg.msg.connect.connect_flags.bits.will_retain =
            client->mqtt_info.lwt_retain ? 1 : 0;
    }
 ...
 *         >= 0: success
 */
int umqtt_publish_ex(struct umqtt_client *client, enum umqtt_qos qos,
                     const char *topic, void *payload, size_t length,
                     int timeout, int retained)
{
    int _ret = 0, _length = 0;
    int _cnt = 0;
    ...
    encode_msg.header.bits.dup = 0;
    encode_msg.header.bits.retain = retained ? 1 : 0;
...
_republish:
    encode_msg.header.bits.dup = (_cnt != 0);
    UMQTT_CLIENT_LOCK(client);
    ...
    if (_length <= 0) 
    {
        UMQTT_CLIENT_UNLOCK(client);
        _ret = UMQTT_ENCODE_ERROR;
        LOG_E(" publish encode failed! topic: %s", topic);
        goto exit;
    }
    client->send_len = _length;
...
_repubrel:
        ...
        if (_length <= 0) 
        {
            UMQTT_CLIENT_UNLOCK(client);
            _ret = UMQTT_ENCODE_ERROR;
            LOG_E(" pubrel encode failed! topic: %s", topic);
            goto exit;
        }
        client->send_len = _length;        
...
int umqtt_publish(struct umqtt_client *client, enum umqtt_qos qos,
                  const char *topic, void *payload, size_t length,
                  int timeout)
{
    return umqtt_publish_ex(client, qos, topic, payload, length,
                            timeout, 0);
}
