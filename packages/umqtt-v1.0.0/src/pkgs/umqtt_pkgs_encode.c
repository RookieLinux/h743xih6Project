...
static int MQTTSerialize_publish(unsigned char* buf, int buflen,
                                 int dup, int qos, int retained,
 ...
 *         >0: package data length
 */
static int umqtt_publish_encode(unsigned char *buf, int buflen,
                                int dup, int qos, int retained,
                                struct umqtt_pkgs_publish *message)
{
    return MQTTSerialize_publish(buf, buflen, dup, qos, retained, message);
}
...
int umqtt_encode(enum umqtt_type type, rt_uint8_t *send_buf, size_t send_len, struct umqtt_msg *message)
{
    ...
    case UMQTT_TYPE_PUBLISH:
        _ret = umqtt_publish_encode(send_buf, send_len,
                                    message->header.bits.dup,
                                    message->header.bits.qos,
                                    message->header.bits.retain,
                                    &(message->msg.publish));
        break;
