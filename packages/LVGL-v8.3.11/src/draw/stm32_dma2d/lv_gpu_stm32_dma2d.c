...
{
    /*
     * Keep mask/alpha blending in software on Cortex-M7.  Fonts, anti-aliased
     * polygons and the music spectrum use short-lived A8 masks which are not
     * guaranteed to be cache-line aligned.  Let DMA2D accelerate the safe,
     * high-bandwidth cases: opaque fills and opaque buffer/image copies.
     */
...
LV_STM32_DMA2D_STATIC void _lv_gpu_stm32_dma2d_await_dma_transfer_finish(lv_disp_drv_t * disp_drv)
{
    ...
    if(isDma2dInProgess) {
        /*
         * DMA2D writes SDRAM behind the Cortex-M7 D-Cache.  The destination
         * cache lines were cleaned before starting DMA2D, so invalidating
         * them now is safe and makes the result visible to later SW drawing.
         */
        __lv_gpu_stm32_dma2d_invalidate_cache(DMA2D->OMAR, DMA2D->OOR,
