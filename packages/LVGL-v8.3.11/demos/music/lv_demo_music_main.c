...
static lv_obj_t * create_handle(lv_obj_t * parent)
{
    ...
    lv_obj_set_style_pad_all(cont, 10, 0);  /* Add padding for better touch area */

    /*A handle to scroll to the track list - make it a larger clickable button*/
    lv_obj_t * handle_btn = lv_btn_create(cont);
    lv_obj_remove_style_all(handle_btn);
    lv_obj_set_size(handle_btn, lv_pct(100), LV_SIZE_CONTENT);
    lv_obj_set_style_pad_all(handle_btn, 8, 0);  /* Button padding */
    lv_obj_add_event_cb(handle_btn, handle_click_event_cb, LV_EVENT_CLICKED, NULL);

    lv_obj_t * handle_label = lv_label_create(handle_btn);
